...
- `lead`：1-2 句白话，先给变化和对读者的影响，必要时补一句来源边界；`lead_claim_ids` 绑定证据账本。架构、评测缩写、effort 档位和单价放到后文。
...
`source_bias_declaration`、`content_gap`、`source_notes` 以及抓取时间等运行元数据是内部编辑与交付字段，供门禁、审计和任务回报使用，不渲染进面向读者的 HTML/Markdown。成品首屏只显示主题和标题；`lead` 供门禁和内部编辑使用，不渲染进读者 HTML/Markdown。页脚只保留来源名称；不展示 URL，也不做成可跳转链接。
...
1. 导语回答“发生了什么，对普通读者意味着什么”，不把说明书术语堆进首屏。
...
面向客户时，整页使用官方简报口吻：清楚、克制、书面，不用口语聊天腔。先说能否使用、成本如何、适合做什么。架构拆分、评测缩写、effort 档位、token 账单、oracle 路由和复现脚本等内部口径不进入读者正文；需要保留证据时，写进 omitted_claims，并标明不适合对客户展示。读者成品也不要出现“辅文称”“仓库称”“材料未核验”等来源处理词。

首屏标签与深度解读同步：使用同一组 3-5 个归档标签，不另造更短口语句。优先主体、产品或模型、机构、技术领域和应用领域。
